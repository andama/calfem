...
class MainWindow(Qt.QMainWindow):
    ...
    def start(self,nodes,elements):
        ...
        for i in range(nnode):
            self.plotter.add_mesh(nodes[i])
            #pv.plot()
            #self.vp += nodes[i]

        nel = np.size(elements, axis = 0)
        for i in range(nel):
            self.plotter.add_mesh(elements[i])
            #self.vp += elements[i]

        #actor = pv.create_axes_orientation_box(line_width=1, text_scale=0.53, edge_color='black', x_color='k', y_color=None, z_color=None, xlabel='X', ylabel='Y', zlabel='Z',color_box=False, labels_off=False, opacity=1.0)
        self.plotter.add_axes()
        self.plotter.add_axes_at_origin(labels_off=True)
        self.plotter.add_floor()
        #self.plotter.add_plane_widget(callback((1,0,0),(0,0,0)))
...
class beam3d:
    def geometry(edof,coord,dof):
        ...
        for i in range(nnode):
            print("test")
            nodes.append(pv.Sphere(center=(coord[i,0],coord[i,1],coord[i,2]), radius=0.03, direction=(0,1,0)))
        ...
        for i in range(nel):
            ...
            elements.append(pv.Cylinder(center=(x,y,z), radius=0.02, direction=(dx,dy,dz), height=h))
